...
if ! command -v nginx &>/dev/null; then
...
elif ! systemctl is-active nginx -q 2>/dev/null; then
    fail "nginx is installed but not running"
    ...
    if [ -f "$SNIPPET" ]; then
        pass "nginx snippet installed: $SNIPPET"
    else
        fail "nginx snippet missing: $SNIPPET"
        echo "         Fix: sudo cp $INSTALL_DIR/nginx.conf $SNIPPET"
    fi

    # Verify the snippet is actually included by some server config
    INCLUDE_FOUND=false
    for f in \
    ...
    do
        [ -f "$f" ] || continue
        if grep -q "$SERVICE_NAME" "$f" 2>/dev/null; then
            pass "Snippet included in: $f"
            INCLUDE_FOUND=true
            break
        fi
    done
    if ! $INCLUDE_FOUND; then
        fail "Snippet not included in any nginx config"
        echo "         Fix: add 'include snippets/$SERVICE_NAME.conf;' inside your server{} block"
    fi
fi
...
http_check "nginx /rf-analyzer/"           "$BASE_URL/rf-analyzer/"           "200" "RF Path Coverage"
...
if [ "$FAIL" -gt 0 ]; then
    ...
    echo "  Snippet missing:         sudo cp $INSTALL_DIR/nginx.conf /etc/nginx/snippets/$SERVICE_NAME.conf"
    echo "  Include not injected:    add 'include snippets/$SERVICE_NAME.conf;' inside your nginx server{} block"
